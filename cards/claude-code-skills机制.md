...
Skills 的答案是**按需加载**：与其把所有知识常驻在上下文中，不如把它们封装成可独立触发的能力单元。当模型判断当前任务涉及某个特定领域时，再加载对应的知识与操作流程。
...
换成企业视角来理解：Tools 是员工的操作工具，SubAgents 是岗位分工，Hooks 是质检流程，`CLAUDE.md` 是企业文化与通用规章。而 **Skills，正是企业的 SOP 体系**——在具体任务发生时按需查阅，并按照步骤执行，输出标准化结果。当员工执行代码审查时，不会即兴发挥，而是参考《代码审查 SOP》，按步骤检查，输出符合模板的报告。

这意味着，当组织的"做事方式"被封装为可语义调用的能力单元，经验就不再依附于老员工的记忆，也不再散落在文档系统中——它变成了模型可以理解、选择和继承的结构。
...
其他文件都是可选的，用来构建更强大的能力——模板文件约束输出格式，示例文件告知 Claude 期望结果，脚本文件让 Claude 实际执行操作。你需要在 `SKILL.md` 中引用这些支持文件，Claude 才知道它们的用途以及何时加载。这是"渐进式披露架构"的体现：不是把所有知识一股脑灌给 AI，而是用到什么加载什么。
...
在早期版本中，斜杠命令 `/Commands` 和 Skills 是两个独立组件。在新版 Claude Code 中，**Commands 已合并到 Skills，成为 Skills 的子集**。Skills 目录的额外优势是支持辅助文件目录（模板、示例、脚本等），能力更强。

一句话区分 `CLAUDE.md` 和 Skills：**`CLAUDE.md` 放"Claude 每次都该知道的规则"；Skill 放"特定场景下的详细指令和知识"。**
