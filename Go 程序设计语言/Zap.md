...
- ```plain
  ...
  +-------------+-----------------------+-------------------------+
  | 格式        | Console (文本+颜色)    | JSON (结构化)            |
  | 适用场景     | 人眼查看（排错）       | 机器解析（ELK/Loki）     |
  | 级别        | Debug 及以上          | Info 及以上              |
  | 堆栈跟踪     | Warn 及以上           | Error 及以上             |
  +-------------+-----------------------+-------------------------+
  ```
- **Development 输出样例：**
    - `2024-11-09T14:03:21.123+0800 DEBUG main.go:10 连接数据库失败` (带颜色，直观)
- **Production 输出样例：**
    - `{"level":"info","ts":1731138201.123,"caller":"main.go:10","msg":"用户登录成功"}` (标准 JSON)
